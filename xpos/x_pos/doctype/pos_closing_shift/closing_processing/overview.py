...
def get_closing_shift_overview(pos_opening_shift: str | dict):
	...
	cash_mode_of_payment = frappe.db.get_value("POS Profile", pos_profile, "cash_mode_of_payment")
...
def get_payment_reconciliation_details(closing_shift_doc: frappe._dict):
	...
	cash_mode_of_payment = (
		frappe.db.get_value("POS Profile", closing_shift_doc.pos_profile, "cash_mode_of_payment") or "Cash"
	)
	...
	return frappe.render_template(  # nosemgrep: frappe-ssti \u2014 template path is a static app-owned file, not user-supplied
		"xpos/x_pos/doctype/pos_closing_shift/closing_shift_details.html",
		{
