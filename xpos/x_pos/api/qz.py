...
def _read_text(path: str) -> str:
	with (
		open(  # nosemgrep: frappe-security-file-traversal — path is always _key_path() or _cert_path(), both within _qz_dir()
			path, encoding="utf-8"
		) as file
	):
		return file.read()
...
def setup_qz_certificate() -> dict[str, str]:
	...
	with (  # nosemgrep: frappe-security-file-traversal — key_path is _key_path(), always within _qz_dir()
		open(key_path, "wb")
	) as file:
		file.write(
			key.private_bytes(
	...
	with (
		open(  # nosemgrep: frappe-security-file-traversal — cert_path is _cert_path(), always within _qz_dir()
			cert_path, "wb"
		) as file
	):
